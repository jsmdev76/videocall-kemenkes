...
export const loader = async ({
	...
	if (data.data.isAccepted) {
		...
		return setUsername(
			...
			'client',
			request,
			`/${roomName}/room?role=client&username=${data.data.clientName}`
		)
	}
	...
	useEffect(() => {
		const handleStatusAndNavigation = async () => {
			if (data.data.callStatus === 1 && remainingTime < 60) {
				...
				alert(
					'Mohon maaf tenaga medis belum tersedia untuk saat ini. Silahkan coba beberapa saat lagi.'
				)
				try {
				...
				{data.data && (
					<div className="wording-call-box">
						...
						<h1 className="text-3xl">
							Menghubungkan anda dengan konselor <br></br>{' '}
							<b>{data.data.agentName}</b>...
						</h1>
