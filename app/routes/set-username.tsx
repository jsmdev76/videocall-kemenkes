...
export const action = async ({ request, context }: ActionFunctionArgs) => {
    ...
    if (formType === 'consultationRequest') {
        ...
        if (Object.keys(errors).length > 0) {
            return json<ActionData>({ success: false, message: 'Form tidak valid. Silakan periksa kembali.' });
        }
        ...
        if (!response.ok) {
            return json<ActionData>({ success: false, message: payload.message || 'Terjadi kesalahan, silakan coba lagi.' });
        }

        return json<ActionData>({ success: true, message: 'Permintaan konsultasi berhasil dikirim!' });
        ...
        if (data.success) {
            ...
            return json<ActionData>({ success: false, message: data.message || 'Terjadi kesalahan saat memulai sesi.' });
        }
    }

    return json<ActionData>({ success: false, message: 'Tipe formulir tidak dikenal.' });
};
...
export default function SetUsername() {
    ...
    return (
        <div className="grid h-full gap-4 place-content-center bg-login">
            ...
            {statusMessage && (
                <div className={`text-${data?.success ? 'success' : 'danger'}`}>
                    {statusMessage}
                </div>
            )}
